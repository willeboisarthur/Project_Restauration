...
PorteurCodeBarres::~PorteurCodeBarres()
{
    wxString codeString = wxString::Format("%d", Code);

    // Check if the length of the string is exactly six characters
    if (codeString.length() == 6)
    {
        // Check if all characters are digits
        if (codeString.IsNumber())
        {
            // Code contains exactly six digits
            return true;
        }
    }
